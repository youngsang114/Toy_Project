...
public interface StoreMenuRepository extends JpaRepository<StoreMenuEntity,Long> {
    ...
    @Query("SELECT sm FROM StoreMenuEntity sm JOIN FETCH sm.storeEntity WHERE sm.id = :id AND sm.status = :status ORDER BY sm.id DESC")
    Optional<StoreMenuEntity> findFirstByIdAndStatusOrderByIdDescWithFetchJoin(@Param("id") Long id, @Param("status") StoreMenuStatus status);
